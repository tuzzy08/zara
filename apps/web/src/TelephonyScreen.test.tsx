...
describe("TelephonyScreen", () => {
  ...
  it("loads saved workflows for the active organization and saves imported-number routes there", async () => {
    ...
    const phoneTestLink = await screen.findByRole("link", {
      name: "Launch Phone test for +14155557890",
    });
    expect(phoneTestLink.getAttribute("href")).toBe(
      `/sandbox?mode=phone-test&workflow=${workflow.id}&number=${phoneNumber.id}`,
    );
  });
});
