...
The public landing page uses a monochrome, editorial signal-system direction distinct from the tenant app's restrained operational shell. Its foundation is near-black and warm-white panels, precise grid lines, oversized condensed display type, monospaced operational labels, hand-drawn annotation accents, and full-width product evidence. Avoid pastel gradients, glass cards, generic SaaS blobs, and stock dashboard compositions on the public route.

Landing copy should position Zara as the system for designing and operating end-to-end voice workflows. Lead with the operating model: strategy, workflow design, telephony, tools, human handoff, observability, and ongoing optimization. The page sequence should move from a cinematic switchboard hero into a manifesto, capabilities, outcomes, workflow-builder proof, live telemetry, operating approach, platform features, integrations, proof, FAQ, and a decisive final CTA. The static hero artwork is a replaceable media layer so a future approved motion version can be substituted without restructuring the hero.

Landing pricing uses the same editorial signal grid. Present approved subscription plans as a precise rate sheet, with one inverted featured plan, followed by a full-width prepaid PAYG row. Do not use floating SaaS pricing cards, gradients, or invented discounts. Show base fees, included runtime, overage rates, the $5 credit pack, and concise billing conditions. Keep a visible Pricing anchor in the primary and footer navigation.

Motion should feel mechanical and controlled. The hero combines its approved switchboard poster with a DOM-built routing console whose dials, switches, signal paths, meters, and status lamps move independently; the poster's slow drift is atmospheric rather than the primary motion. The console route map uses one responsive SVG coordinate system for its nodes, ports, and paths so every edge terminates at a visible port. Cyan identifies call input, amber identifies routing, violet identifies the active agent, coral identifies tools, and green identifies human transfer and live state. Intermediate nodes expose ports on both ends. The rest of the page uses a continuous twelve-column editorial grid, a precision Zara glyph family built on a shared 48-unit optical grid with rounded terminals and restrained secondary detail, dense workflow and telemetry evidence, scanning lines, waveform activity, and restrained view-linked reveals. Marketing pages must not use stock Lucide icons as their visual identity. Always provide a reduced-motion fallback. The tenant app remains an operational product surface; `/login` and `/signup` keep their compact dedicated auth treatment and must not expand into duplicate marketing pages.

The marketing workflow preview follows the same geometry rule as the hero route map. Nodes, ports, and edges must share one SVG coordinate system. Every edge must terminate at the center of a visible port. On small screens, preserve a horizontally inspectable canvas instead of reducing node labels below a readable size.

Workflow builder controls should stay compact and task-first. The canvas opens as a blank draft by default, restores an in-progress session draft when the operator returns, and loads saved workflows only through the toolbar selector. The workflow selector is the only always-visible workflow identity control in the toolbar; workflow naming belongs in the publish dialog. Inspector controls should reveal only settings that match the active runtime, and multi-value fields such as supported languages should use dropdown-style checkbox selection instead of tall native listboxes. Agent inspectors keep only Agent details expanded by default; other panels start collapsed, panel summaries show a compact required-info indicator when required details are missing inside, and section chevrons use the orange affordance with hover ripple feedback.

Sandbox and telephony testing should keep one operator mental model. Use explicit mode labels: Published test (browser) and Phone test (Twilio/PSTN). `Run in sandbox` on `/workflows` must publish or use an unchanged published workflow before opening the inline browser test drawer. `/calls` can launch Phone test for a routed number, `/workflows` can deep-link to it, and `/sandbox` owns the full Phone test surface. While a Phone test is waiting or active, the start control remains visibly green and disabled, with the end control as the only termination action. Do not reintroduce a draft-runtime sandbox or a second routed-number sandbox inside the workflow drawer.

The full `/sandbox` page keeps Live cost and Session metrics as the only persistent right-rail cards. Escalations, live monitor, replay, routing, assigned tools, and published manifest remain available through one compact tabbed utility dock beneath the primary session surface; they must not return as a tall stack of equal-weight cards. Runtime routing details belong under the Routing utility and should not use a separate Runtime decision card.
...
- Present configured providers in a compact Connections table. Connection operations use slightly enlarged icon-and-text buttons with readable labels and functional colored outlines: green for heartbeat, orange for validation, blue for number import, and red for deletion.
