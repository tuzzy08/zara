...
Motion should feel mechanical and controlled: slow image drift, short interface transitions, scanning lines, waveform activity, and restrained view-linked reveals. Always provide a reduced-motion fallback. The tenant app remains an operational product surface; `/login` and `/signup` keep their compact dedicated auth treatment and must not expand into duplicate marketing pages.
