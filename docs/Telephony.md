...
After a terminal call state is stored, the telephony service sends a server-owned fact to the trusted billing producer. Tenant requests cannot create billing usage or supply a price. The ledger key includes the tenant, call session, meter class, and route so duplicate terminal callbacks do not create duplicate usage.

Platform-managed carrier duration uses provider-connected seconds and rounds up to a full minute for each route. BYO calls do not create a platform carrier charge. Runtime remains measured in seconds. Failed calls that never connect create an explicit zero-charge fact. Missing rate or provider-connected duration creates an incomplete fact for review. All customer charge delivery stays in shadow mode until ISSUE-248.
