...
- Follow-up on 2026-06-11: fixed a workflow sandbox start race where the browser could post a real actor user ID for `workspace-support` before auth context finished repairing/confirming workspace access. The workflow toolbar now disables `Run in sandbox` until the selected workspace is locally accessible or the server-owned active workspace has been returned by auth context, and the start path also guards against direct invocation.
