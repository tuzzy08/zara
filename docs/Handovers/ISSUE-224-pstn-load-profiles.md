...
- Run the explicitly approved stepped, failure, burst, and soak jobs from an external generator against an isolated staging resource shape.
- Retain the current single-instance machine reports under the approved baseline artifact store before incremental persistence migration begins.
...
Provision the isolated staging target and external generator, then run and retain the approved pre-persistence baseline profiles documented in `apps/pstn-protocol-simulator/README.md`.
