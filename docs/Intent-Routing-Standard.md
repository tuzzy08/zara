...
- Router agents triage through configured handoff targets; tenants do not need a separate classifier agent to make that decision.
...
- Agent-attached routing is agent-decided through an internal handoff tool/action, not a separate classifier turn.
...
Agent-attached route policies let an active speaking router agent keep the conversation until it decides the caller should be handed off to one configured target agent. Operators do not need to draw a separate intent node and handoff node for this common pattern.

The active router agent owns the practical intent decision by choosing whether to call Zara's internal handoff tool/action. Zara injects only compact handoff targets derived from the compiled manifest: concrete agent IDs, configured agent names, role/class context, and fallback posture. The agent never receives graph target IDs, connector credentials, provider URLs, arbitrary target-entry fields, or tenant-authored branch descriptions/examples.

The runtime owns validation and execution. It accepts only configured target agent IDs, ignores any model-supplied graph target, validates target existence, transfer loops, language support, and fallback posture, then has the source agent speak the caller-facing handoff announcement before activating the target agent. Agent-target routes create `AgentTransferContext` and may write an `IntentRouteResult`-compatible fact for observability, but no extra classifier model call is required for agent-attached routing.

The tenant builder keeps one Agent node runtime model. The builder presents Agent and Router Agent as distinct toolbox presets and inspector experiences, but Router Agent still creates a normal agent node with route policy enabled. Normal Agents do not expose a route/behavior conversion dropdown. Router Agents retain normal tools, knowledge, voice, language, runtime, and prompt configuration; routing is an additional capability, not a replacement for tool use. Route target options are derived from actual named agent nodes in the current workflow, excluding the source agent. Branch labels may be edited for operator readability, but branch descriptions/examples and specialist prompt templates are platform-admin owned. Fallback can clarify with the source agent or choose an existing configured target. The canvas may show compact badges such as Routes, but it must not add separate tenant-managed triage, handoff, or intent-route node types for this path.

Platform-admin runtime controls govern global defaults and review posture for this feature: internal handoff tool naming, announcement mode, fallback posture, validation/audit posture, and role/class routing profiles. Staff can save these defaults through guarded platform-admin route-policy APIs with expected-version checks and audit reasons. Tenant-facing builder controls choose per-workflow handoff targets and labels, while platform-admin remains the staff governance surface for default route-policy behavior.
...
Agent-attached route policies are preserved in draft and compiled manifests under the current schema. The runtime projects them to the active router agent as concrete handoff targets plus an internal `handoff_to_agent` / `zara_handoff_to_agent` tool, validates any requested target against saved policy, ignores model-supplied graph targets, and has the source agent speak the configured handoff announcement before target activation. Standalone legacy intent routes remain classifier-backed until a future removal slice.
