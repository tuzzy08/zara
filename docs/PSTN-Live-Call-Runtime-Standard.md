...
ISSUE-149 implements the premium realtime PSTN path in the provider-neutral core contracts and the Nest telephony runtime. A premium route resolves to `pstn-premium-realtime` only after provider capability, provider availability, tenant entitlement, budget posture, and explicit fallback-policy checks pass. For production execution, Twilio's authorized Media Stream loads the dispatch's exact persisted published manifest, creates the platform-policy-selected premium session, forwards inbound G.711 mu-law through the provider adapter, and writes provider output back as Twilio media. OpenAI Realtime is the default and uses native `audio/pcmu` input/output for Twilio. Gemini Live converts Twilio PCMU to provider-native PCM16 16 kHz input and converts provider PCM16 24 kHz output back to PCMU. Provider-native interruption clears Twilio buffered audio, tool/handoff messages continue through the shared premium runtime loop, and provider failure closes both legs without a silent sandwich downgrade.

ISSUE-184 hardens that path with one in-memory actor per active premium PSTN call. The actor owns `initializing`, acknowledged `ready`, `active`, `draining`, reserved `handing_off`, `stopped`, and `failed` lifecycle states. OpenAI waits for `session.updated` and Gemini waits for `setupComplete`; early caller media remains bounded to 5,250 ms and 256 KiB of serialized resident provider payload and flushes in order only after acknowledgement. All premium calls share a process-wide 32 MiB startup/handoff admission bound. Provider WebSocket pressure is capped at 256 KiB, Twilio message processing is bounded to 64 KiB of pending ingress, and retained bridge event history is capped. Overflow, congestion, readiness failure, provider error, Twilio stop, and application shutdown close both call legs and release runtime ownership idempotently.

ISSUE-185 makes OpenAI premium playback deterministic. OpenAI `response.created` registers provider response ownership before audio; native PCMU deltas are split into exact 160-byte/20 ms frames with cumulative timestamps and a final partial frame is padded with mu-law silence only after `response.output_audio.done`. A 50-mark in-flight window bounds Twilio's remote playback buffer while up to 30 seconds/240,000 audio bytes wait locally; all active premium calls share a separate 32 MiB queued-playback admission bound, and provider-message ingress is independently capped at 64 KiB and 256 messages. Every frame has a playback mark, and a separate completion boundary is emitted only after provider audio completion. Twilio mark echoes release window slots and aggregate playback admission, then complete a response only after every frame and boundary is acknowledged. Interruption or terminal cleanup releases all remaining admission; interruption also advances the generation, clears Twilio once, invalidates old response ownership, ignores clear-returned marks and late old deltas, and accepts audio only from a subsequently registered response. Mark stream SIDs must match the active stream.

ISSUE-186 makes premium PSTN handoff provider-aware. Runtime resolves the target concrete agent's provider, model, voice, prompt, and tools into a replacement session while preserving the logical Zara session and packet context. OpenAI handoff announcements carry transfer-scoped response metadata so only the matching provider response and acknowledged Twilio playback boundary can release the swap. The actor buffers at most 5,250 ms or 256 KiB of serialized target-native provider payload during transition under the shared process admission bound; the target continuation is sent before that media flushes. Provider-leg epochs reject every late source callback, and a five-second timeout, replacement readiness failure, actor overflow, caller stop, or shutdown closes all owned legs without silently selecting `pstn-sandwich`.
...
ISSUE-220 supersedes the earlier channel-selected VAD and one-second ingress defaults. A versioned platform-admin conversation policy resolves provider, model, fixed channel media contract, and provider-native turn/activity settings once at call start. OpenAI is the default with model `gpt-realtime-2.1`; PSTN uses semantic VAD with low eagerness, automatic response creation, and interruption enabled. Gemini keeps provider-native activity handling. Zara performs no local VAD and premium execution consumes normalized caller/assistant lifecycle events rather than raw provider event names. Startup and handoff each admit at most 5,250 ms or 256 KiB of serialized resident provider payload, with a process-wide 32 MiB aggregate admission limit. OpenAI interruption clears Twilio playback and sends `conversation.item.truncate` only when Twilio has acknowledged played 20 ms frame marks, using the exact provider item/content identity and acknowledged duration. Later media for a terminal execution is ignored with a stable terminal result.
...
Premium realtime over PSTN uses provider-owned turn/activity detection and normalized Zara caller/assistant lifecycle events. Zara clears provider playback ownership but does not implement a second VAD. For OpenAI, caller activity during response playback clears Twilio audio and truncates only the unheard remainder from provider conversation state using acknowledged Twilio playback marks. Gemini interruption remains provider-native. Premium handling must not duplicate or partially fork the PSTN sandwich v1 barge-in logic.
