...
ISSUE-185 makes OpenAI premium playback deterministic. OpenAI `response.created` registers provider response ownership before audio; native PCMU deltas are split into exact 160-byte/20 ms frames with cumulative timestamps and a final partial frame is padded with mu-law silence only after `response.output_audio.done`. A 50-mark in-flight window bounds Twilio's remote playback buffer while at most 40,000 audio bytes wait locally; provider-message ingress is independently capped at 64 KiB and 256 messages. Every frame has a playback mark, and a separate completion boundary is emitted only after provider audio completion. Twilio mark echoes release window slots and complete a response only after every frame and boundary is acknowledged. Interruption advances the generation, clears Twilio once, invalidates old response ownership, ignores clear-returned marks and late old deltas, and accepts audio only from a subsequently registered response. Mark stream SIDs must match the active stream.
