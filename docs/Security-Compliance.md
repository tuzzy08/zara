...
- Production Better Auth config must use secure cookies behind the Coolify reverse proxy, trusted proxy headers, database-backed rate limiting through the durable Better Auth `rateLimit` table, and reset-time session revocation.
