...
| Premium OpenAI Realtime route announcement is not spoken before handoff | The source Router Agent must speak the handoff before Zara switches roles. If OpenAI returns a bare route tool call, Zara sends the tool output and asks the current source role to say the exact configured announcement, stores the validated route as pending, and applies the handoff only after that announcement response completes. If the same OpenAI response already included a spoken message before the route tool call, Zara can apply the handoff immediately and the target agent must not repeat the announcement. |
