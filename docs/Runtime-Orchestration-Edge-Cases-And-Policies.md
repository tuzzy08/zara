...
| Premium OpenAI Realtime route announcement is not spoken after a route tool call | When route policy announcement text is configured, route tool handling sends the tool output and follows with a `response.create` instruction that conditionally speaks the announcement if the router did not already say it, then continues as the target agent in the same response. |
