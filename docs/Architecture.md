...
- Production telephony persistence has separate ownership boundaries. Configuration workflows serialize the tenant configuration aggregate for provider connections, imported numbers, route configuration, health posture, heartbeats, and encrypted credential envelopes. Live-call webhooks, dispatches, execution sessions and commands, media tokens, phone-test checkpoints, call-control events, lifecycle transitions, policy changes, handoffs, and termination use tenant-and-call scoped incremental Postgres operations only. Database constraints, transactions, and compare-and-swap versions are the runtime concurrency authority; there is no live-call snapshot or in-memory durability fallback.
