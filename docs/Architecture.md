...
OpenAI Realtime speech-to-speech is the default premium realtime provider for calls or nodes that need very low latency, natural turn-taking, or high-value treatment. Provider and model selection are platform-admin policy; tenant builders select only the runtime profile and receive a read-only provider-aware voice catalog. Google Gemini Live is also available as a server-owned premium realtime provider option; browser clients still receive Zara-controlled session transports rather than direct provider URLs or credentials, and stale provider/model fields in tenant workflow graphs cannot override platform policy.
