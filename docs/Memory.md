...
Source snapshots default to the active workspace and may include workflow IDs. Provider imports require a connected provider that supports knowledge sources plus an active `knowledge-source` grant for the selected workspace/workflow. Intercom Articles, Confluence spaces/pages, SharePoint sites/pages/folders, Freshdesk Solutions categories/folders/articles, and Salesforce Knowledge articles/categories resolve provider content server side from stable source selections, create record-level review drafts with source URIs, and never expose provider API URLs or auth headers to tenant users. Freshdesk selections use `category:<categoryId>`, `folder:<folderId>`, or `article:<articleId>`; Salesforce Knowledge selections use `article:<articleId>` or `category:<dataCategoryGroup>:<dataCategoryName>`. Published runtime manifests carry the workflow and workspace identity so retrieval can use the frozen allowed scope while newly approved records inside that scope become available to new calls.
...
Website crawl refresh compares approved page URLs and content hashes against the latest crawl. Provider knowledge-source refresh uses the same review-gated pattern for Confluence, SharePoint, Freshdesk, and Salesforce Knowledge selections: added records create new drafts, changed records create update drafts, and removed source URLs create deletion drafts while approved records stay active until deletion approval. Provider 401/403 failures degrade the source, pause future refresh, and keep approved snapshots active. Runtime retrieval never performs live website, Confluence, SharePoint, Freshdesk, or Salesforce Knowledge search during calls.
