...
The tenant and platform-admin Vite apps use `packages/auth-client` as their shared Zara auth boundary. The package configures requests against `VITE_AUTH_BASE_URL` or `VITE_API_BASE_URL`, falling back to the local Nest API origin, and exposes normalized `useSession`, `getContext`, email/password sign-up, email/password sign-in, password reset request/reset submit, email verification request, safe session list/revoke, invitation create/list/revoke/accept, organization selection, and sign-out methods for the apps. App-shell reads use the server-owned auth context as the read model. Browser auth mutations call the mounted Better Auth REST endpoints directly with cookies; the frontend does not bundle Better Auth React hooks or organization-reader clients for normal shell operation.
...
Production auth config enables secure cookies, trusted reverse-proxy headers, password-reset session revocation, and Better Auth rate limiting with database storage. The durable auth schema includes Better Auth's `rateLimit` table with `key`, `count`, and `lastRequest` so production auth endpoints do not depend on process memory. Zara's default global auth bucket is 300 requests per 60 seconds so normal shell/session/org reads do not lock out active users, while Better Auth's stricter built-in sign-in, sign-up, password-reset, and verification-email limits remain in force. `BETTER_AUTH_SECRET` must be at least 32 characters outside tests. `ZARA_AUTH_EMAIL_WEBHOOK_URL` is required in production; test uses an in-memory auth email journal and local development logs queued email links.
...
The API includes `@polar-sh/better-auth` and `@polar-sh/sdk`. `apps/api/src/billing/better-auth-polar.ts` defines the Better Auth Polar plugin composition for checkout, portal, usage, and webhooks, while `BillingService` owns Zara's tenant billing state and the public API contract. Polar webhook handling processes `customer.state_changed`, `order.paid`, `order.refunded`, and `subscription.past_due`. The public endpoint reads `webhook-id`, `webhook-timestamp`, and `webhook-signature`. When `POLAR_WEBHOOK_SECRET` is configured, the SDK verifies the original request bytes captured by Nest's `rawBody` option, not re-serialized JSON. Missing raw bytes and invalid signatures are rejected. Schema failures return a safe 400 response without the raw SDK error. Customer-state events use `data.id` for the Polar customer and `data.external_id` for the Zara tenant; `data.organization_id` is the Polar merchant, not the tenant. Processed webhook IDs suppress exact replay. A processing error marks the receipt failed. An exact later delivery can reopen that failed receipt and retry processing. Local and unit-test fixtures retain the existing unset-secret bypass; never expose a local webhook without its secret.
