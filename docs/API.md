...
The tenant and platform-admin Vite apps use `packages/auth-client` as their shared Zara auth boundary. The package configures requests against `VITE_AUTH_BASE_URL` or `VITE_API_BASE_URL`, falling back to the local Nest API origin, and exposes normalized `useSession`, `getContext`, email/password sign-up, email/password sign-in, password reset request/reset submit, email verification request, safe session list/revoke, invitation create/list/revoke/accept, organization selection, and sign-out methods for the apps. App-shell reads use the server-owned auth context as the read model; Better Auth browser client calls are reserved for explicit auth mutations such as sign-in, sign-out, and organization selection.
