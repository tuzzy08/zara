...
Concrete runtime agents may additionally carry `realtimeProvider` and optional `realtimeModelId` only after platform-admin/runtime policy projection; tenant graph values are stripped before those defaults are applied. Platform prompt-policy agent class defaults can explicitly override the global premium conversation-policy provider/model for a concrete specialist. Otherwise `openai-realtime` and `gpt-realtime-2.1` are the defaults; `gemini-live` selects the server-owned Gemini Live pattern. Premium realtime session responses expose the resolved immutable provider/model/media/turn-policy version and a Zara-owned transport URL, not provider credentials or Google/OpenAI WebSocket endpoints. Provider replacement during handoff resolves the target from the same call-start conversation-policy snapshot.
