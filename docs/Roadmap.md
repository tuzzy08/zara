...
PSTN live call runtime planning note: `docs/PSTN-Live-Call-Runtime-Standard.md` defines the ISSUE-142 through ISSUE-149 standard. ISSUE-142 is implemented as the shared provider-neutral live call session core, ISSUE-143 is implemented as the provider-neutral `pstn-sandwich` media harness for cost-optimized and balanced calls, ISSUE-144 is implemented as the first Twilio bidirectional Media Streams bridge, and ISSUE-145 is implemented as protected phone-test routing. Phone numbers now store separate `liveRoute` and `testRoute` records instead of legacy flat route fields; `testRoute` requires a published version, allowed caller numbers, and an expiring waiting session. Premium realtime over PSTN is explicitly blocked until the later ISSUE-149 slice.
...
Next PSTN sequencing note: ISSUE-146 through ISSUE-149 extend the protected phone-test backend into the unified sandbox UX, activation, observability, and premium realtime follow-up work. `/calls` remains the setup and activation ladder, while `/workflows` and `/sandbox` can initiate the unified Phone test mode against exact published versions before a route is promoted live.
