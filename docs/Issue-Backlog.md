...
- Publishable workflow draft: ISSUE-011 through ISSUE-014, ISSUE-016, and ISSUE-017. Implemented baseline: connector-aware tool nodes, condition routes, exit nodes, escalation lanes, immutable version publishing, and draft runtime manifest preview. ISSUE-182 supersedes specialist handoff nodes with router-agent route policies and concrete agent handoff actions.
...
Acceptance criteria:
- Published workflow sandbox runtime decision display reflects a selected Gemini Live realtime provider/model instead of showing the profile-level OpenAI Realtime default.
- Premium realtime published runs suppress stale sandwich-routing copy such as OpenAI standard profile-default decisions when the effective agent runtime is Gemini Live or OpenAI Realtime.
- Start Call actions are disabled while the live sandbox is connecting, active, listening for user speech, or playing agent audio.
...
Acceptance criteria:
- The tenant sandbox model exposes clear modes: Published test (browser) and Phone test (Twilio/PSTN), with labels that make the correct mode obvious
...
- 2026-09-03 local sandbox preparation: isolated Postgres/Redis, all 37 migrations, builds, and 22 focused tests passed. The sandbox token works but has no products. No payment test or public tunnel has run. Tunnel setup is paused after Defender blocked the official ngrok download; the ZIP matches the publisher checksum, but this is not a malware clearance. See the handover for evidence and remaining setup.
- The user approved Cloudflare as the replacement. Cloudflare 2026.8.3 has a matching published checksum and valid publisher signature. Four test-first checks passed for the local webhook-only filter. Tool policy blocked the public tunnel launch; waiting for the user-started tunnel URL. No security restriction was bypassed.
- The user subsequently supplied a working Cloudflare URL. The sandbox webhook and ignored local secret are configured; the isolated API readiness check passed. The endpoint and public forwarding remain disabled. A signed local HTTP diagnostic confirmed Zara rejects Polar's standard webhook headers with 400 `Polar webhook id is required.` Handler repair awaits user approval; original-body signature verification also needs a regression test. No payment qualification has passed.
- 2026-09-04: The user approved the webhook repair. Standard headers, native customer-state identity, original-body signature verification, and safe schema errors are fixed with test-first coverage. The complete billing test group passed: 50 files, 309 tests, 3 PostgreSQL-only tests skipped. API build passed. The old tunnel URL now fails DNS resolution; a new user-started URL is required. The sandbox endpoint remains disabled. No real Polar delivery or payment qualification is claimed.
- The replacement tunnel URL is now saved in Polar with the endpoint disabled. Docker Desktop fails during Inference manager startup because it cannot access its `dockerInference` file. The local Linux engine, API, and proxy are not ready. Docker repair needs user approval; no reset or file removal was attempted.
- The user restored Docker. The isolated database/Redis and corrected API are ready. Signed transport checks passed locally and through the public tunnel; other API routes return 404. Enabled the sandbox endpoint. Polar rejected the example.com test email with 422 before customer creation; waiting for the user's valid sandbox customer email. No real webhook delivery or payment qualification has passed yet.
- The user then supplied a valid sandbox email. Created the sandbox customer and verified Polar's real `customer.state_changed` delivery (201). Provider redelivery returned 200 with `replay=true`; Postgres still contains exactly one processed receipt with the original processing time. Customer-state delivery is verified. Catalog/product setup, checkout, paid-order, credit-grant, and refund qualification remain pending. No live billing setting changed.
- Created a private, one-time USD 5.00 Polar sandbox PAYG product and mapped it to `payg-5-usd` in the isolated local database with a clearly test-only catalog. Created checkout `83d532a4-7531-4053-846a-5551296a9b83` through Zara's compiled Polar client. The sandbox form is prepared with Polar's public test card and is waiting for user confirmation before **Pay now**. Payment, credit-grant, replay, and refund evidence remain pending. No production setting changed, and the test catalog is not a release approval.
- Completed the real sandbox PAYG path after user confirmation. Polar paid order `f1ea2afe-1b7d-4e6a-b0a3-14d798a52a7a` created one 500-cent grant; provider replay added nothing. The real refund exposed and drove test-first fixes for Polar's separate 465-cent net plus 35-cent tax refund fields and for reopening failed receipts. After rebuild and exact provider redelivery, Zara recorded one reversal; refund replay added nothing; final balance is zero cents. API build passed. The full billing suite passed 50 files and 310 tests with 3 PostgreSQL-only tests skipped. No production setting or live charge changed. Subscription sandbox checks and all non-Polar release evidence remain pending.
- Prepared private monthly Polar sandbox products for Starter at USD 49, Growth at USD 149, and Scale at USD 499, and mapped them in the isolated test-only catalog. The local API and webhook proxy are ready, but the previous Cloudflare Quick Tunnel no longer resolves. No subscription customer, checkout, or payment was created without a working public webhook route. Subscription qualification resumes after the user starts a replacement tunnel.
- The user started a replacement tunnel and confirmed the three sandbox subscription submissions. Starter, Growth, and Scale now each have one Polar-confirmed active subscription and one matching paid Zara invoice for 4,900, 14,900, and 49,900 USD cents. All 12 subscription-path receipts are processed without error. Redelivery of each plan's paid-order and latest customer-state event created no duplicate customer, subscription, invoice, or receipt. Polar sandbox states that payments are not processed, so no real money moved. Polar PAYG and subscription qualification is complete; usage delivery and all non-Polar release evidence remain pending.
- The user reports that the deployed secret store now contains the Cartesia admin key and OpenAI Admin key. Provider reads have not yet verified these values. Gemini billing evidence is deferred; a Gemini-backed tenant cannot qualify until Google Cloud Billing export and Application Default Credentials are configured. Cartesia and OpenAI still need immutable tenant scope mappings.
...
- Completion is blocked on deployed runtime report credentials and real completed-cycle provider reports, internal and selected-tenant canaries, and the required billing, security, and release approvals. Twilio evidence is connected through existing encrypted tenant credentials. Polar sandbox evidence is complete.
