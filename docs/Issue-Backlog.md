...
- Integration registry and knowledge-source expansion: ISSUE-156 and ISSUE-157 are implemented; ISSUE-158 is in progress; ISSUE-159 through ISSUE-170 are pending. Current baseline: capability-based provider registry, tenant-safe API-served catalog foundation, tenant frontend consumption of the catalog for current provider/tool surfaces, scoped connection/grant foundations, and backend publish blocking for invalid agent-tool grants. Planned follow-ups add the remaining scoped setup UX, provider contract and side-effect safety, knowledge-base source import plus daily sync, and the first popular provider expansion batch.
...
- Added backend workflow publish validation plus a published sandbox startup guard for connector tool bindings with missing grants, revoked/unavailable connections, missing provider scopes, or missing role-specific `agent-tool` coverage.
- Remaining work in this issue is the tenant builder publish API wiring plus capability toggles/presets/setup-copy UX for knowledge-source and post-call sync grants.
