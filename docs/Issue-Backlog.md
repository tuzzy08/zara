...
- Integration registry and knowledge-source expansion: ISSUE-156 is implemented; ISSUE-157 through ISSUE-170 are pending. Current baseline: capability-based provider registry and tenant-safe API-served catalog foundation. Planned follow-ups add scoped connection/grant UX, provider contract and side-effect safety, knowledge-base source import plus daily sync, and the first popular provider expansion batch.
