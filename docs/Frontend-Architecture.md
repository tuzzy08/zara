...
Toolbelt assignment surfaces separate integration selection from provider tool selection. The inspector shows the Integration selector before the Tools selector, labels choices with provider names such as Zendesk or HubSpot instead of account/domain previews, and then loads only that selected provider connection's catalog tools into a checkbox-style multi-select. Built-in provider tools are grouped by connector, for example Zendesk lists Search tickets, Create ticket, and Update ticket from the backend connector schema. Publish creates missing workspace/integration/agent/tool-scoped agent-tool grants only after connection, workspace, provider, and scope validation pass, and runtime execution still requires those explicit grants. Cross-provider tool access uses separate toolbelt assignments, not separate visual tool nodes. The integration selector binds an assignment to a tenant integration credential fetched from the integrations API; it is not an API endpoint selector. Built-in provider API URLs, paths, auth headers, and payload shapes remain Zara-owned connector metadata and are not editable in the workflow builder. The integrations page does not expose a workflow-scoped grant editor.
...
- `/workflows` opens a blank draft canvas by default even when saved workflows exist. Operators load or delete saved workflows explicitly from the toolbar selector, and in-progress canvas state is preserved for the current organization/workspace browser session when navigating away and back.
...
- `/integrations` loads OAuth connections, health posture, connector tools, webhook HTTP tools, and workspace tool grants from the Nest integrations APIs. Connect, credential rotation/reconnect, health-check, and delete actions call backend routes and only display masked credential references. Provider rows and catalog tools use accessible provider logo badges so Zendesk, HubSpot, Google Workspace, Microsoft 365, Notion, Salesforce, Slack, and webhook tools remain scannable without loading remote brand assets.
