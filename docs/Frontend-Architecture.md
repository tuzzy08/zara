...
- `/workflows` `Run in sandbox` opens the publish flow when the current graph is unpublished or dirty, and opens the inline sandbox drawer only for an unchanged published workflow version.
...
- The workflow-page sandbox runtime card resolves premium realtime display from the effective entry agent provider/model, so Gemini Live published runs show Gemini Live instead of the profile-level OpenAI Realtime default or stale sandwich text-routing decisions.
