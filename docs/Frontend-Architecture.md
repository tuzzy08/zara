...
The runtime orchestration standard is now captured in `docs/Intent-Routing-Standard.md`, `docs/Agent-Tool-And-Transfer-Standard.md`, and `docs/Turn-Runtime-Packet-v1.md`. Agent-attached route policies now replace visible tenant-managed intent and handoff nodes for the common route-after-agent workflow. Tools behave as optional agent capabilities, and runtime routes create receiving-agent context without exposing internal packet details to operators.
