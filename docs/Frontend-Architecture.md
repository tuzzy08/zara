...
The planned PSTN live call runtime keeps one sandbox concept instead of creating a second competing sandbox. The tenant UI should expose explicit modes: Draft test (browser), Published test (browser), and Phone test (Twilio/PSTN). `/calls` remains the setup and activation ladder for numbers, routes, and subscription state; `/workflows` and `/sandbox` can initiate or deep-link Phone test mode for exact published workflow versions. ISSUE-145 adds the backend/API baseline for protected Phone tests: phone numbers expose `liveRoute`, `testRoute`, and `phoneTestResults`, while legacy flat phone-number route fields are gone. Number states should read as Unassigned, Test route, Ready to activate, Live, and Paused. See `docs/PSTN-Live-Call-Runtime-Standard.md`.
